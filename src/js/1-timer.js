...
flatpickr('#datetime-picker', {
  ...
  onClose(selectedDates) {
    ...
    if (stampNow < stampSel) {
      ...
      switchBtn('On');
      iziToast.success({
        message: 'You have selected a correct date in the future!',
      });
    } else {
      timer.stop();
      this.setDate(dtNow);
...
const timer = {
  ...
  start(selectedTime) {
    if (this.intervalId) {
      iziToast.info({
        message: 'Timer in progress! To restart, refresh this page.',
      });
      return;
    }
    ...
    Object.keys(this.dtObj).forEach(it => {
      this.pageElems[it].textContent = this.dtObj[it]
        .toString()
        .padStart(2, '0');
    });
  ...
  updValues() {
    ...
    Object.keys(this.dtObj).forEach(it => {
      if (this.dtObj[it] != newTimeObj[it]) {
        ...
        this.pageElems[it].textContent = newTimeObj[it]
          .toString()
          .padStart(2, '0');
      }
    });
